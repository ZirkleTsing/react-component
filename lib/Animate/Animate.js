...
export default class Animate extends React.Component {
  ...
  componentWillReceiveProps (nextProps) {
    ...
    if (showProp) {
      currentChildren.forEach(currentChild => {
        ...
        if ((!nextChild || !nextChild.props[showProp]) && currentChild.props[showProp]) {
          ...
          newChild = nextChild
          // When currentChild is false, this situation does not need to modify nextChild,
          // because there will be no jitter, only when current is true and next is not or false, then you need to temporarily set true,
          // and waiting for the subsequent process to enqueue the components of the error state,
          // finally render the correct components and set the correct state in a life cycle.
        }
        if (newChild) {
    ...
    currentChildren.forEach(child => {
      ...
      if (showProp) {
        const showInNow = child.props[showProp] // bug show in now false
        if (hasNext) {
